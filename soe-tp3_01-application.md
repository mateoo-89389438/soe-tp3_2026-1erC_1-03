...
- **Mecanismo de Señalización mediante Semáforo Binario:** Se declaró e inicializó un semáforo binario global denominado `xSemaphore_Producer_Consumer` en `app.c`. Al crearse de manera nativa con `xSemaphoreCreateBinary()`, el token se inicializa en 0 (vacío), lo que garantiza el bloqueo preventivo del consumidor en caso de que no existan recursos disponibles al arrancar el sistema.
